...
/**
 * Middleware validating Shopify inventory endpoint requests.
 * Inventory represents CURRENT catalog stock; date parameters are strictly OPTIONAL.
 */
const validateShopifyInventoryRequest = (req, res, next) => {
  const query = req.query || {};
  const body = req.body || {};
  const params = req.params || {};
  ...
  // 2. Extract date parameters if optionally provided
  const datePreset = (query.datePreset || query.date_preset || "").trim();
  const dateFrom = (query.dateFrom || query.date_from || "").trim();
  const dateTo = (query.dateTo || query.date_to || "").trim();
  ...
  if (datePreset && !ALLOWED_SHOPIFY_PRESETS.includes(datePreset.toLowerCase())) {
    return sendError(
      res,
      400,
      `Unsupported date_preset '${datePreset}'. Allowed presets: ${ALLOWED_SHOPIFY_PRESETS.join(", ")}`
