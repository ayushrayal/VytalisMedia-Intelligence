...
import { getAdSetBreakdowns } from "../services/meta.api.js";
...
/**
 * AdSetBreakdowns Component.
 * AdSet-scoped breakdown viewer supporting Age, Gender, and Placement.
 * Scoped directly by adSetId to guarantee exact Ad Set metrics.
 */
export const AdSetBreakdowns = ({ adSetId, dateParams = {}, currency = "INR", titlePrefix = "Ad Set" }) => {
  ...
  const fetchBreakdownData = useCallback(async () => {
    ...
    try {
      setLoading(true);
      setError(null);
      const res = await getAdSetBreakdowns(adSetId, {
        ...dateParams,
  ...
  }, [adSetId, activeCategory, dateParams]);

  // Reset state and immediately trigger fresh fetch whenever adSetId, category, or dateParams change
  useEffect(() => {
    setRows([]);
    fetchBreakdownData();
  }, [fetchBreakdownData]);
  ...
  if (!adSetId) {
    return (
      ...
      >
        Please select an Ad Set to view performance breakdowns.
      </div>
    );
  }
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "12px" }}>
        <h4 style={{ margin: 0, fontSize: "15px", fontWeight: "700", color: "#0F172A" }}>
          {titlePrefix} Performance Breakdowns
        </h4>
      ...
      {loading ? (
        ...
        >
          No {activeCategory} breakdown data available for this Ad Set and date range.
        </div>
      ) : (
        <div
          style={{
...
export default AdSetBreakdowns;
