...
import { getAdBreakdowns } from "../services/meta.api.js";
...
/**
 * AdBreakdowns Component.
 * Ad-scoped breakdown viewer supporting Age, Gender, and Placement.
 * Scoped directly by adId to guarantee exact Creative / Ad metrics.
 * Prevents stale data when switching creatives via request-isolation refs.
 */
export const AdBreakdowns = ({ adId, dateParams = {}, currency = "INR" }) => {
  const [activeCategory, setActiveCategory] = useState("age"); // "age" | "gender" | "placement"
  const [rows, setRows] = useState([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);

  // Active ad ID ref to prevent out-of-order race conditions when switching creatives
  ...
  const fetchBreakdownData = useCallback(async () => {
    const currentAdId = adId ? String(adId).trim() : "";
    if (!currentAdId) {
      setRows([]);
      setLoading(false);
      return;
    ...
    try {
      ...
      const res = await getAdBreakdowns(currentAdId, {
        ...dateParams,
        breakdown: activeCategory,
      });

      // Ignore out-of-order response if adId changed mid-flight
      if (activeAdIdRef.current !== currentAdId) return;
      ...
      if (activeAdIdRef.current === currentAdId) {
        setLoading(false);
      }
    }
  }, [adId, activeCategory, dateParams]);

  // Reset state immediately on adId change to prevent stale data display
  useEffect(() => {
    setRows([]);
    fetchBreakdownData();
  }, [fetchBreakdownData]);
  ...
  const cleanAdId = adId ? String(adId).trim() : "";
  if (!cleanAdId) {
    return (
      <div
        style={{
      ...
      >
        No valid Meta Ad ID available for this creative.
      </div>
    );
  }
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
      ...
      {loading ? (
        ...
        >
          No {activeCategory} breakdown data available for this ad and date range.
        </div>
      ) : (
        <div
          style={{
