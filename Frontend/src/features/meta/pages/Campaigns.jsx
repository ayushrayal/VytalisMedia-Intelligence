            ...
            <tbody>
              {data.map((row, idx) => (
                <tr key={idx} style={{ borderBottom: "1px solid #334155" }}>
                  <td style={{ padding: "10px", fontWeight: "600" }}>{row.campaign || row.campaign_name || "-"}</td>
                  <td style={{ padding: "10px" }}>
                    <span style={{ padding: "2px 8px", borderRadius: "4px", fontSize: "0.75rem", backgroundColor: "#334155", color: "#818cf8" }}>
                      {row.campaign_status || row.campaign_effective_status || "ACTIVE"}
                    </span>
                  </td>
                  <td style={{ padding: "10px" }}>{row.campaign_objective || "-"}</td>
