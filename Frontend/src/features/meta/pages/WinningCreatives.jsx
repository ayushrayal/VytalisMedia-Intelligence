...
import { getNormalizedStatus } from "../components/StatusBadge.jsx";
...
import WinningCreativesSection from "../components/WinningCreativesSection.jsx";
...
/**
 * Dedicated Winning Creatives Page Component.
 * Route: /meta/winning-creatives
 * Features:
 * - Displays creatives performing above the selected ROAS threshold (sorted highest ROAS first)
 * - Shared account, date range, spend, and status filter controls
 * - Dynamic ROAS threshold selector with frame-1 persistence across reloads
 * - Reuses existing CreativeCard, CustomizeCardsModal, and CreativeDetailsDrawer
 */
export const WinningCreatives = () => {
  ...
  // Fetch creative performance records using existing getCreatives API
  ...
  // Sync preferences on mount if no local storage preference exists
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle="Creatives performing above the selected ROAS threshold."
        actions={
      ...
      {isDisplayLoading ? (
        <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
          <ContextualLoader isLoading={loading} onComplete={handleComplete} section="meta-winning" minHeight="auto" />
        ...
        <WinningCreativesSection
          creatives={filteredData}
          preferences={cardPreferences}
          onCardClick={handleCardClick}
          onThresholdChange={handleWinningThresholdChange}
        />
      )}
...
export default WinningCreatives;
